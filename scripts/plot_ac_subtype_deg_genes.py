...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Plot AC subtype DEG top1 gene UMAPs.")
    parser.add_argument("--h5ad", default=DEFAULT_H5AD, help="Input AC AnnData h5ad file.")
    parser.add_argument(
        "--gene-csv",
        nargs="+",
        default=[
            "results/tables/ac_subtype_gene_umaps/ac_subtype_deg_max_other_top1_marker_per_cluster.csv",
            "results/tables/ac_subtype_gene_umaps/ac_subtype_deg_specific_top1_marker_per_cluster.csv",
        ],
...
def save_sample_highlight_umaps(adata, condition_key: str, figures_dir: Path, dpi: int) -> None:
    ...
    for sample in ordered_samples:
        ...
        fig.savefig(
            figures_dir / f"ac_umap_by_{condition_key}_{sample}_highlight.png",
            dpi=dpi,
            bbox_inches="tight",
        )
        plt.close(fig)
...
def main() -> None:
    ...
    pd.DataFrame(records).to_csv(tables_dir / "ac_deg_top1_gene_match_report.csv", index=False)

    save_overview_umap(
        adata,
        args.condition_key,
        figures_dir / f"ac_umap_by_{args.condition_key}.png",
        args.dpi,
    )
    ...
    save_overview_umap(
        adata,
        args.cluster_key,
        figures_dir / f"ac_umap_by_{args.cluster_key}.png",
        args.dpi,
    )
    ...
    for gene in present_genes:
        plot_gene_all_conditions(
            adata,
            gene,
            figures_dir / f"ac_umap_gene_{gene}_all_conditions.png",
            args.dpi,
        )
        plot_gene_by_condition(
            ...
            args.condition_key,
            condition_panels,
            figures_dir / f"ac_umap_gene_{gene}_split_by_condition.png",
            args.dpi,
        )

    print(tables_dir / "ac_deg_top1_gene_match_report.csv")
